...
@section('content')
<div class="container-fluid py-4"><div class="card"><div class="card-header pb-0"><h6>{{ __('messages.update') }} {{ __('messages.local_unit') }}</h6></div><div class="card-body"><form method="POST" action="{{ route('admin.local-units.update',$localUnit) }}">@csrf @method('PUT') @include('admin.local-units.partials.form',['localUnit'=>$localUnit])<button class="btn btn-primary btn-sm">{{ __('messages.update') }}</button></form></div></div></div>
@endsection
@push('scripts')
<script>
(function(){
